...
prova.describe("s10 ci parity", function()
	...
	prova.test("golang and rust stacks mirror their inline community-action steps", function(t)
		local go = p6m.ci.dockerfile(p6m.ci.stacks.golang)
		t:expect(go):matches("^FROM golang:1%.23\n")
		t:expect(go):matches("RUN go build %./%.%.%.\nRUN go test %./%.%.%.\n")
		local rust = p6m.ci.dockerfile(p6m.ci.stacks.rust)
		t:expect(rust):matches("^FROM rust:1\n")
		t:expect(rust, "build, test, then clippy as -D warnings — the rendered order")
			:matches("RUN cargo build\nRUN cargo test\nRUN cargo clippy %-%- %-D warnings\n")
	end)
end)

...
prova.describe("s10 ci parity", function()
	...
	prova.test("python stack mirrors python-uv-setup + python-uv-build", function(t)
		...
		t:expect(df):matches("^FROM ghcr%.io/astral%-sh/uv:python3%.12%-bookworm\n")
	...
	prova.test("java stack mirrors the rendered build.yaml's maven verify override", function(t)
		...
		t:expect(df):matches("^FROM maven:3%-eclipse%-temurin%-21\n")
		t:expect(df):matches("RUN mvn verify %-%-no%-transfer%-progress\n")
	end)

	prova.test("golang stack tidies before its inline community-action steps", function(t)
		local go = p6m.ci.dockerfile(p6m.ci.stacks.golang)
		t:expect(go):matches("^FROM golang:1%.23\n")
		t:expect(go, "tidy first — a fresh render ships no go.sum")
			:matches("RUN go mod tidy\nRUN go build %./%.%.%.\nRUN go test %./%.%.%.\n")
	end)
	...
	prova.test("rust stack mirrors rust-setup + rust-build defaults, protoc first", function(t)
		...
		t:expect(rust):matches("^FROM rust:1\n")
		t:expect(rust, "protoc before any cargo — prost/tonic build scripts need it")
			:matches("RUN apt%-get update && apt%-get install %-y protobuf%-compiler\nRUN cargo fmt")

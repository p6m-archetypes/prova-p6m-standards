...
prova.describe("s10 ci parity", function()
	...
	prova.test("golang stack generates guarded gRPC bindings, then tidies, then builds", function(t)
		local go = p6m.ci.dockerfile(p6m.ci.stacks.golang)
		t:expect(go):matches("^FROM golang:1%.23\n")
		t:expect(go, "codegen guarded on proto/ and pinned to the Dockerfile's plugin versions")
			:matches("RUN if %[ %-d proto %]; then.*protoc%-gen%-go@v1%.36%.5.*protoc%-gen%-go%-grpc@v1%.5%.1.*; fi\n")
		t:expect(go, "codegen before tidy — tidy must see the generated local package")
			:matches("; fi\nRUN go mod tidy\nRUN go build %./%.%.%.\nRUN go test %./%.%.%.\n")
	end)
